...
func maxMinutes(guardTimes map[string][]int) (string, []int) {
	sleepiestGuard := ""
	var minutes []int
	for k, v := range guardTimes {
		if len(v) > len(minutes) {
			sleepiestGuard = k
			minutes = v
		}
	}
	return sleepiestGuard, minutes
}
...
func main() {
	...
	sleepiestGuard, minutes := maxMinutes(guardTimes)
	fmt.Println(sleepiestGuard)
	mode, _ := mode(minutes)
	guardId, _ := strconv.Atoi(sleepiestGuard)
	fmt.Println(guardId * mode)
}
