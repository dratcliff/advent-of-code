...
def day_nineteen():
    rules, messages = parse("Day19p2.txt")
    rules = to_rule_map(rules)
    for k, v in rules.items():
        print(k, v)
    
    rules["0"] = ["31"]
    p31 = rules_to_possibilities(rules)
    rules["0"] = ["42"]
    p42 = rules_to_possibilities(rules)
    ct = 0
    messages = [m for m in messages if len(m) > 0]
    for m in messages: 
        found = True
        if len(m) % 8 != 0 or len(m) < 16:
            found = False
        chunks = [m[i:i+8] for i in range(0, len(m), 8)]
        print(chunks)
        if chunks[0] in p42 and chunks[-1] in p31:
            forty_twos = 1
            thirty_ones = 1
            chunks = chunks[1:-1]
            while len(chunks) > 0 and chunks[0] in p42:
                chunks = chunks[1:]
                forty_twos += 1
            while len(chunks) > 0 and chunks[-1] in p31:
                chunks = chunks[:-1]
                thirty_ones += 1
            if len(chunks) != 0:
                found = False
            if forty_twos <= thirty_ones:
                ...
                print("forty twos", forty_twos, "thirty ones", thirty_ones)
        else:
            found = False
        if found:
            ct += 1
